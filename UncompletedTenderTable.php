...
function printPDF(){
    ...
    const url = `PrintNotSubmitted.php?organization=${encodeURIComponent(customer)}&user=${encodeURIComponent(filterUser)}&startDate=${startDate}&endDate=${endDate}`;
...
function loadSalesData(page = 1) {
 currentPage = page;
    ...
    let url = `UncompletedTenderTableDone.php?page=${page}&limit=5&organization=${encodeURIComponent(customer)}&user=${encodeURIComponent(filterUser)}`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            data.data.forEach(row => {
                ...
                `;
                tr.onclick = () => window.location.href = `ViewUncompletedTender.php?id=${row.id}`;
